...
public class MyPanel extends JPanel {
	...
	private boolean search(ArrayList<Integer>Listtobesearched,int value)
	{
		for(int i=0;i<Listtobesearched.size();i++)
			if(value== Listtobesearched.get(i))
				return true;
		return false;
	}
	public MyPanel(boolean orient)
	{
		...
		Sortare.setBackground(Color.white);
		Sortare.setBounds(100,360,180,50);
		Sortare.setFont(new Font("Arial", Font.BOLD, 15));
...
while(!stack.empty())
{
	...
	for(int i=1;i<listaAdiacenta.get(s-1).size();i++)
	{
 		if(listaAdiacenta.get(s-1).get(i)!=0)
			if(visited.elementAt(listaAdiacenta.get(s-1).get(i))==0)
			stack.push(listaAdiacenta.get(s-1).get(i));
	}
}
	}
}
